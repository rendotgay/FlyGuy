...
logger = Logger("db", color=color_from_hex("ff6308", bold=True))
    ...
    try:
        ...
        if count == 0: # if old_gifs table exists, also check that
            try:
                ...
                count = cursor.fetchone()[0]
                db.close()
                logger.log("CHECKING GIF SUCCESSFUL")
                return count == 0
            except sqlite3.OperationalError:
                logger.error("ERROR CHECKING GIF")
                return True
        else:
            return False
    except sqlite3.OperationalError:
...
def insert_gif(user1: int, user2: int, url):
    ...
    try:
        ...
        logger.error(f"ERROR INSERTING GIF: {e}")
        create_table()
...
def increment_rarity(user_id: int, rarity: str) -> None:
    ...
    try:
        ...
        logger.error("ERROR INCREMENTING RARITY. IS THIS EVEN STILL USED?")
        create_table()
...
def increment_xp(user_id: int, roll: str, multiplier: float = 1.0) -> int:
    ...
    if roll == 'common':  # 65% - Common
        ...
        xp = 0
        logger.error(f"ROLL ERROR: {roll}")
    xp *= multiplier
    xp = math.ceil(xp)
    logger.log(f"XP earned: {xp}")
    try:
        cursor.execute('''
        ...
        logger.error("ERROR INCREMENTING XP")
        create_table()
...
def check_user_count(inviter_id, target_id):
    db = get_db()
    cursor = db.cursor()
    cursor.execute(
        """
...
def get_user_invite_totals(user_id: int) -> tuple[int, int]:
    db = get_db()
    cursor = db.cursor()
    try:
        cursor.execute('''
        SELECT
            (SELECT COUNT(*) FROM gifs WHERE user1 = ?),
            (SELECT COUNT(*) FROM gifs WHERE user2 = ?)
        ''', (user_id, user_id))
        return cursor.fetchone()
    except sqlite3.OperationalError:
        logger.error("ERROR GETTING INVITE TOTALS")
        create_table()
        return get_user_invite_totals(user_id)


def get_bot_invite_totals():
    db = get_db()
    cursor = db.cursor()
    cursor.execute('''
                   SELECT COUNT(*)
    ...
    try:
        ...
        return counts + old_counts
    except sqlite3.OperationalError:
        logger.error("No old gifs table.")
        return counts